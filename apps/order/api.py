...
class OrderAPIView(GenericViewSetCustom):
    ...
    @list_route(methods=['GET'])
    @Core_connector()
    def getOrderInfo(self,request):

        try:
            order = Order.objects.filter(orderid=request.query_params_format.get("orderid"))
        except Order.DoesNotExist:
            raise PubErrorCustom("订单号不存在!")

        if order.status != '1':
            raise PubErrorCustom("支付状态异常!")

        return {"data":order.jd_data}
